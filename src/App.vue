...
<button @click="$event => something($event)">Click me</button>

<dialog ref="dialogbox">
  
  
  <h1>{{ text }}</h1>
  <button @click="$event => somethingNot($event)">Click me</button>

    <div v-for="(q, qindex) in questions"
         class="question_div"
         :id="`q${qindex + 1}`">
      ...
      <div class="ratingDiv">
        <span class="uncolor"
              v-for="(i, rindex) in stars"
              :id="`q${qindex + 1}r${rindex + 1}`"
              @click="colorTillhere(qindex + 1, rindex + 1)"
              :style="{ filter: coloredStars[qindex + 1] && coloredStars[qindex + 1] >= rindex + 1 ? 'grayscale(0%)' : 'grayscale(100%)' }">

          <svg class="rate_icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" width="48px" height="48px"><linearGradient id="q0c2dLEp_4LHk~8cW2fATa" x1="9.009" x2="38.092" y1="6.36" y2="45.266" gradientUnits="userSpaceOnUse">
